    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-8">
        <section className="card-base">
          ...
          {loading ? <p className="mt-4 text-slate-400">Carregando post...</p> : null}
          {error ? <p className="mt-4 text-sm text-red-600">{error}</p> : null}

          {!loading && !error && post && !isOwner ? (
            <div className="mt-4 rounded-2xl border border-slate-700 bg-slate-900 p-4 text-slate-100">
              <p>Voce nao pode editar este post porque ele pertence a outro usuario.</p>
              <Link to="/" className="mt-3 inline-block font-medium text-devconnect-300 hover:text-devconnect-200">
                Voltar para o feed
              </Link>
            </div>
          ) : null}
